...
for filepath in glob.glob("listing/*/index.html"):
    ...
    domain = official_url.replace("https://", "").replace("http://", "").replace("www.", "").strip("/")
    
    tw_url = f"https://twitter.com/search?q={urllib.parse.quote(domain)}"
    ig_url = f"https://www.instagram.com/explore/search/keyword/?q={urllib.parse.quote(domain)}"
    fb_url = f"https://www.facebook.com/search/pages/?q={urllib.parse.quote(domain)}"
...
print(f"Fast processed {count} listings.")

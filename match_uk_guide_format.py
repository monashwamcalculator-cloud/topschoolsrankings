...
def build_html_page(slug, title, meta_title, meta_desc, html_content, schema_json, featured_src, featured_alt):
    page = f"""<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
  ...
  <main>
    ...
    <header class="page-header">
      <div class="site-container narrow">
        ...
        <h1>{title}</h1>
        <p>{meta_desc}</p>
        <div class="page-meta">Published 25 August 2026 · Reviewed by the TSR Editorial Desk · Official sources linked</div>
      </div>
    </header>
...
for filename, metadata in content_map.items():
    ...
    full_html = build_html_page(slug, h1_title, meta_title, meta_desc, html_content, schema_json, featured_src, featured_alt)
    ...
    print(f'Re-generated formatting for /{slug}/ matching the UK course guide structure')

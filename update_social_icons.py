...
unis = [
    'purdue-university', 'university-of-florida', 'university-of-wisconsin-madison', 
    'university-of-california-san-diego', 'university-of-maryland-college-park', 
    'ohio-state-university', 'pennsylvania-state-university', 'rutgers-university', 
    'texas-a-and-m-university', 'michigan-state-university'
]

pattern = r'<div><dt>Social Media</dt><dd><a href="([^"]+)"[^>]*>Twitter</a> · <a href="([^"]+)"[^>]*>Instagram</a> · <a href="([^"]+)"[^>]*>Facebook</a></dd></div>'

for uni in unis:
    path = f"listing/{uni}/index.html"
    ...
    if match:
        ...
        ig_url = match.group(2)
        fb_url = match.group(3)
        ...
        with open(path, "w", encoding="utf-8") as f:
            f.write(content)
        print(f"Replaced with icons for {uni}")
    else:
        print(f"No match in {uni}")

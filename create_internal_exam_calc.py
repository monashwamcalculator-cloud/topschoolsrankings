...
tool_slug = "exam-passing-internal-marks-calculator"
tool_dir = os.path.join("tools", tool_slug)
os.makedirs(tool_dir, exist_ok=True)

html_content = """<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Exam Passing & Internal Marks Calculator | Top Schools Rankings</title><meta name="description" content="Calculate exactly how many marks you need in your final exam to pass or reach your target grade based on your internal marks."><meta name="robots" content="index, follow"><link rel="canonical" href="https://topschoolsrankings.com/tools/exam-passing-internal-marks-calculator/"><meta name="google-adsense-account" content="ca-pub-5825245351059712"><meta property="og:type" content="website"><meta property="og:site_name" content="Top Schools Rankings"><meta property="og:title" content="Exam Passing & Internal Marks Calculator"><meta property="og:description" content="Calculate exactly how many marks you need in your final exam to pass or reach your target grade based on your internal marks."><meta property="og:url" content="https://topschoolsrankings.com/tools/exam-passing-internal-marks-calculator/"><meta property="og:image" content="https://topschoolsrankings.com/og.png"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:image" content="https://topschoolsrankings.com/og.png"><link rel="icon" href="/favicon.jpg"><link rel="stylesheet" href="/assets/site.css"><script async crossorigin="anonymous" src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-5825245351059712"></script><script type="application/ld+json">{"@context":"https://schema.org","@type":"WebPage","name":"Exam Passing & Internal Marks Calculator","description":"Calculate exactly how many marks you need in your final exam to pass or reach your target grade based on your internal marks.","url":"https://topschoolsrankings.com/tools/exam-passing-internal-marks-calculator/","publisher":{"@type":"Organization","name":"Top Schools Rankings","url":"https://topschoolsrankings.com"}}</script><script src="/assets/site.js" defer></script>
<style>
...
@media (min-width: 600px) {
  ...
  <nav class="breadcrumbs site-container" aria-label="Breadcrumb"><a href="/">Home</a><span><i aria-hidden="true">/</i><a href="/tools/">Tools</a></span><span><i aria-hidden="true">/</i><b>Exam Passing Calculator</b></span></nav>
  <header class="page-header"><div class="site-container narrow"><span class="eyebrow">Interactive student resource</span><h1>Exam Passing & Internal Marks Calculator</h1><p>Calculate exactly how many marks you need in your final or external exam to pass a subject, based on the internal/assignment marks you have already scored.</p><div class="page-meta">Free to use A No account A Instant calculation</div></div></header><section class="section site-container article-layout"><article class="article-body">
  
  <section class="tool-panel" data-custom>
    <div class="tool-panel-head" style="margin-bottom: 25px;">
      <span>Smart tool</span>
      <h2>Calculate Final Exam Target</h2>
    </div>
    
    <div class="input-row two-col">
      <div>
        <label>Total Subject Marks (e.g. 100)</label>
        <input type="number" id="total_marks" placeholder="100" min="1">
      </div>
      <div>
        <label>Target % (e.g. 40 for Pass, 80 for 'A')</label>
        <input type="number" id="target_percent" placeholder="40" min="1" max="100">
      </div>
      <div>
        <label>Your Internal Marks So Far</label>
        <input type="number" id="internal_scored" placeholder="e.g. 18" step="0.5" min="0">
      </div>
      <div>
        <label>Final Exam Total Marks</label>
        <input type="number" id="final_exam_total" placeholder="e.g. 70" min="1">
      </div>
    </div>
    
    <button class="calc-btn" onclick="calculatePassing()">Calculate Marks Needed</button>
    
    <div id="result_box" class="tool-result" style="display:none; margin-top:25px; text-align:center; padding:30px;">
      <h3 style="font-size:20px; font-weight:600; opacity:0.9; margin:0;">Marks Needed in Final Exam</h3>
      <div id="result_val" class="result-big">0</div>
  ...
  <script>
    function calculatePassing() {
      ...
      const targetPercentInput = document.getElementById('target_percent');
      const internalInput = document.getElementById('internal_scored');
      const finalExamInput = document.getElementById('final_exam_total');
      const resultBox = document.getElementById('result_box');
      const resultVal = document.getElementById('result_val');
      const resultMessage = document.getElementById('result_message');
      
      let totalMarks = parseFloat(totalMarksInput.value);
      let targetPercent = parseFloat(targetPercentInput.value);
      let internalScored = parseFloat(internalInput.value);
      let finalExamTotal = parseFloat(finalExamInput.value);
      
      if (isNaN(totalMarks) || isNaN(targetPercent) || isNaN(internalScored) || isNaN(finalExamTotal)) {
        alert("Please fill all the numerical fields.");
        return;
      }
      if (internalScored > totalMarks || finalExamTotal > totalMarks) {
        alert("Internal marks and Final Exam total cannot exceed Total Subject Marks.");
        return;
      }
      
      let targetMarks = (totalMarks * targetPercent) / 100;
      let marksNeeded = targetMarks - internalScored;
      ...
      if (marksNeeded <= 0) {
        ...
        resultMessage.innerHTML = `Congratulations! You have already achieved the ${targetPercent}% target based entirely on your internal marks. Any mark you score in the final exam is a bonus!`;
      } 
      else if (marksNeeded > finalExamTotal) {
        resultVal.innerText = "Impossible";
        resultBox.classList.add('impossible');
        let maxPossible = ((internalScored + finalExamTotal) / totalMarks) * 100;
        resultMessage.innerHTML = `Unfortunately, you cannot reach ${targetPercent}%. Even if you score a perfect ${finalExamTotal}/${finalExamTotal} in the final exam, your maximum possible score will be ${maxPossible.toFixed(2)}%.`;
      } 
      else {
        resultVal.innerText = marksNeeded.toFixed(1) + " / " + finalExamTotal;
        resultBox.classList.add('possible');
        resultMessage.innerHTML = `You need to score exactly <strong>${marksNeeded.toFixed(1)}</strong> out of ${finalExamTotal} in the upcoming final exam to hit your ${targetPercent}% target. Good luck!`;
      }
  ...
  <div class="rich-article-content" style="margin-top: 40px;">
    <h2>How to Calculate Marks Needed to Pass?</h2>
    <p>University grading systems are often split between internal assessments (mid-terms, assignments, lab work) and external assessments (the final written exam). When finals approach, every student asks the same question: <em>"How many marks do I actually need to pass?"</em></p>
    <p>Our Internal Marks & Final Exam Calculator is designed specifically for students in India, Asia, and global universities following a split-weightage system (e.g., 30/70, 40/60, 25/75 models).</p>
    ...
    <ul>
      ...
      <li><strong>Your Internal Marks So Far:</strong> Sum up everything you've scored in mid-semesters, vivas, and assignments. (Example: You scored 18 out of 30 in internals).</li>
      <li><strong>Final Exam Total Marks:</strong> The maximum marks of the upcoming theory paper (Example: 70).</li>
    </ul>

    <h2>Understanding the Exam Math</h2>
    <p>Let's say your university uses a 30-70 split (30 Internal, 70 External) with a 40% passing criteria out of 100 total marks. To pass, you need a grand total of 40 marks.</p>
    <p>If you scored a healthy 25/30 in your internals, you only need to score <strong>15 marks out of 70</strong> in the final exam to pass (25 + 15 = 40). This drastically reduces your exam stress!</p>
    <p>However, if you scored a poor 5/30 in internals, you will need to score a massive <strong>35 marks out of 70</strong> in the final exam just to barely pass.</p>

    <h3>Can I fail even if I reach the target?</h3>
    <p><strong>Warning:</strong> Always check your specific university rulebook. Many universities have a "Double Passing" rule. This means you must achieve 40% overall AND a minimum of 40% strictly in the final external exam. For a 70-mark paper, this means you MUST score at least 28 marks regardless of how high your internal marks are.</p>
  </div>
...
print("Created tools/exam-passing-internal-marks-calculator/index.html")

# Let's dynamically add it to tools/index.html under "College & University Academics" using python
...
if container:
    new_card_html = """
<article class="listing-card" style="border-top: 4px solid #ffc107; padding-top: 20px;">
  <svg style="width:32px; height:32px; margin-bottom:15px; color:#ffc107; display:block;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    <path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path>
  </svg> 
  <span>High Demand Tool 🔥</span> 
  <h2><a href="/tools/exam-passing-internal-marks-calculator/">Exam Passing & Internal Marks Calculator</a></h2>
  <p>Calculate exactly how many marks you need in your final exam to pass or reach your target grade based on your internal marks.</p>
  <a href="/tools/exam-passing-internal-marks-calculator/">Use tool →</a>
</article>
"""
    new_card = BeautifulSoup(new_card_html, "html.parser").article
    
    # We find the header "College & University Academics"
    # It's inside a div. We find that div, then insert after.
    header = container.find("h2", string="College & University Academics")
    if header:
        header.parent.insert_after(new_card)
        with open("tools/index.html", "w", encoding="utf-8") as f:
            f.write(str(soup))
        print("Updated tools/index.html (inserted correctly under College category)")
    else:
        print("Could not find the College & University Academics header!")
...
sitemap_entry = """
  <url>
    <loc>https://topschoolsrankings.com/tools/exam-passing-internal-marks-calculator/</loc>
    <lastmod>2026-08-25</lastmod>
  </url>"""

if "exam-passing-internal-marks-calculator" not in sitemap_xml:

...
# We map the filename to the canonical slug from the SEO plan
slug_map = {
...
# Copy all assets (SVGs and WebPs) to /media/articles/
...
# We need a base template. Let's read the index.html of an existing blog
with open(os.path.join(site_dir, "blogs", "top-50-universities-in-usa", "index.html"), "r", encoding="utf-8") as f:
...
for filename, slug in slug_map.items():
    ...
    # Strip the H1 from markdown so it doesn't double-render
    md_text = '\n'.join(lines[1:])
    
    # Simple markdown to HTML conversion with tables
    html_content = markdown.markdown(md_text, extensions=['tables', 'fenced_code'])
    
    # Fix image paths. The markdown probably uses relative paths like `../assets/featured/image.webp`
    # We moved them to `/media/articles/image.webp`
    ...
    # We will inject these cards into the main /blogs/index.html
...
# Handle Redirect for boarding-schools-in-canada
...
if os.path.exists(blogs_index_path):
    ...
    if grid:
        for card_html in blogs_list_cards:
